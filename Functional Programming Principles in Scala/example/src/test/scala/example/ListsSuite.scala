...
class ListsSuite extends FunSuite {
  ...
  test("Negative test for sum of a few numbers") {
    assert(sum(List(1, 2, 0, 5)) !== 3)
  }

  test("Empty list test for sum of a few numbers") {
    assert(sum(List()) === 0)
  }

  test("Negative test for max of a few numbers") {
    assert(max(List(3, 7, 2, 9)) !== 7)
  }

  test("Empty list test for max of a few numbers") {
    Try {
      max(List())
    } match {
      case Success(_) =>
      case Failure(ex) =>
        assertResult("Empty list")(ex.getMessage())
